...
private:
    ...
    static void DoExecute(const std::string& command,
    ...
    static std::string FindPath(TCmdEnvironment& cmdEnv, const std::string& command) {
...
TExecutorPtr TExecutorFactory::MakeExecutor(std::string, TEnvironment& globalEnvironment) {
    return std::make_shared<TExternalExecutor>(globalEnvironment);
}
